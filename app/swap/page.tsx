...
import { MAINNET_RELAY_API, convertViemChainToRelayChain, createClient } from "@relayprotocol/relay-sdk";
import { adaptViemWallet } from "./relayViemWallet";
...
function withLogo(c: RelayCurrency, logoURI: string, extra?: Partial<RelayCurrency>): RelayCurrency {
  return {
    ...
    ...(extra ?? {}),
    logoURI,
    metadata: { ...(c.metadata ?? {}), ...(extra?.metadata ?? {}), logoURI },
  };
}
            ...
            <div>
              ...
              {mounted ? (
                <div className="mt-2 text-xs text-neutral-400 break-all">
                  Wallet: {isConnected ? "connected" : "not connected"} • chainId: {chainId ?? "—"} • address:{" "}
                  {address ?? "—"} • walletClient: {walletClient ? "ok" : "—"} • widgetWallet:{" "}
                  {relayWallet ? "ok" : "—"}
                </div>
              ) : null}
            </div>
                    ...
                    <div>
                      ...
                      {isBaseCard ? (
                        <div className="mt-3 rounded-xl border border-orange-500/20 bg-orange-500/10 p-3 text-xs text-orange-200">
                          <b>Base swap is coming soon.</b> DTC on Base will be announced from official Donald Toad Coin accounts only — beware of impersonators and fake token contracts.
                        </div>
                      ) : null}
                    </div>
