...
type ApiPayload = {
  ...
  tf: Timeframe;
  address: `0x${string}`;
  byChain: Partial<Record<ChainKey, ChainSummary>>;
  total: Omit<ChainSummary, "chain">;
  meta?: any;
};
...
function ttlMs(tf: Timeframe) {
  if (tf === "daily") return 45_000;
  if (tf === "weekly") return 90_000;
  if (tf === "monthly") return 150_000;
  return 240_000;
}

function getKey(tf: Timeframe, address: string) {
  return `ps:${tf}:${address.toLowerCase()}`;
}

function getCache(): Map<string, CacheEntry> {
  const g = globalThis as any;
  if (!g.__PROFILE_SUMMARY_CACHE__) g.__PROFILE_SUMMARY_CACHE__ = new Map<string, CacheEntry>();
  return g.__PROFILE_SUMMARY_CACHE__;
}
...
function emptyChainSummary(chain: ChainKey): ChainSummary {
  return {
    chain,
    games: 0,
    wins: 0,
...
export async function GET(req: Request) {
  try {
    ...
    const byChain: Partial<Record<ChainKey, ChainSummary>> = {};
      ...
      try {
        ...
        if (method === "blockscout" && cfg.blockscoutApi) {
          ...
          gameLogs = await rpcGetLogsSplit(cfg.rpcs, {
            address: cfg.game,
            topics: [TOPIC_GAME_SETTLED, null as any, addrTopic],
            fromBlock: fromHex,
            toBlock: toHex,
          } as any);
          ...
          boundLogs = await rpcGetLogsSplit(cfg.rpcs, {
            address: cfg.registry,
            topics: [TOPIC_BOUND, null as any, addrTopic],
            fromBlock: fromHex,
            toBlock: toHex,
          } as any);
          ...
          claimedLogs = await rpcGetLogsSplit(cfg.rpcs, {
            address: cfg.registry,
            topics: [TOPIC_CLAIMED, null as any, addrTopic],
            fromBlock: fromHex,
            toBlock: toHex,
          } as any);
        }

        const s = emptyChainSummary(chainKey);
        ...
        for (const l of gameLogs) {
          ...
          s.volumeDtc += toDtc2(amountReceived);
          s.profitDtc += toDtc2(playerNetWin);
          const winDtc = toDtc2(playerNetWin);
          if (winDtc > s.topWinDtc) s.topWinDtc = winDtc;
        ...
        for (const l of claimedLogs) {
          ...
          s.claimedDtc += toDtc2(amount);

          claimedCount += 1;
        }

        s.referrals = referees.size;
    ...
    for (const ck of Object.keys(byChain) as ChainKey[]) {
      ...
      total.volumeDtc += s.volumeDtc;
      total.profitDtc += s.profitDtc;
      if (s.topWinDtc > total.topWinDtc) total.topWinDtc = s.topWinDtc;
    ...
    const payload: ApiPayload = {
      ...
      meta: {
        ...
        limitations: {
          pendingGames: "Not available without a GameCreated event (only GameSettled exists).",
          claimableEpochCredit: "Not available from logs only; requires registry getters or credit events.",
          modeAndBet: "Mode not emitted; wager approximated by amountReceived in GameSettled.",
        },
      },
    };
