...
const CHAIN: Record<ChainKey, { rpc: string; game: `0x${string}`; registry: `0x${string}` }> = {
  base: {
    rpc: process.env.NEXT_PUBLIC_BASE_RPC_URL || process.env.BASE_RPC_URL || "https://mainnet.base.org",
    game: "0x05df07E37B8dF836549B28AA3195FD54D57DD845",
    registry: "0x994a28Bb8d84AacB691bA8773e81dAFC1acEb39B",
  },
  linea: {
    rpc: process.env.NEXT_PUBLIC_LINEA_RPC_URL || process.env.LINEA_RPC_URL || "https://rpc.linea.build",
...
function toDtcInt(n: bigint) {
  return Number(n / 10n ** DTC_DECIMALS);
}
...
async function findBlockByTimestamp(client: any, chainKey: ChainKey, targetSec: number, side: "lte" | "gte") {
...
async function getLogsPaged(client: any, args: { address: `0x${string}`; fromBlock: bigint; toBlock: bigint; topics?: Hex[] | Hex[][] }) {
  const maxSpan = 5000n;
  const out: any[] = [];
  ...
  while (from <= to) {
    const end = from + maxSpan > to ? to : from + maxSpan;
...
export async function GET(req: Request) {
  try {
    ...
    const agg = new Map<string, { chains: Set<ChainKey>; games: number; volume: bigint; topWin: bigint; profit: bigint; referrals: Set<string>; claimed: bigint }>();
